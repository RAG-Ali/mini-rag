...
async def index_project(request : Request,project_id : str , push_request : PushRequest):
    ...
    chunck_model = await ChunckModel.create_instance(db_client = request.app.db_client)
    ...
    while has_records:
        page_chuncks = await chunck_model.get_project_chuncks(project_id = project.id,page_no=page_no)
        if len(page_chuncks):
            page_no+=1
        ...
        chunck_ids = list(range(idx,idx+len(page_chuncks)))
