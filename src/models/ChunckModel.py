...
from .db_schemas import DataChunck
...
class ChunckModel(BaseDataModel):
    ...
    async def insert_chunck(chunck : DataChunck):
    ...
    async def get_chunck(self,chunck_id: str):
        ...
        data_chunck=data_chunck(**result)

    async def insert_many_chuncks(self,chuncks : list,batch_size : int =100):
        
        for iterator in range(0 , len(chuncks),batch_size):
            batch = chuncks[iterator:iterator+batch_size]
            operations = [
    ...
    async def get_project_chuncks(self,project_id : ObjectId , page_no : int = 1 , page_size : int = 50):
        
        records = await self.collection.find({
            "chunck_project_id": project_id
        }).skip((page_no -1) * page_size).limit(page_size).to_list(length = None)
        
        return [DataChunck(**record) for record in records]
